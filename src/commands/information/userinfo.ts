...
export async function run(client: DaClient, interaction: CommandInteraction, args: Collection<string, ArgsInterface>) {
    ...
    const member = memberID ? (await guild?.members.fetch({ user: memberID, withPresences: true }) || interaction.member as (GuildMember | null)) : interaction.member as (GuildMember | null);
    ...
    { guildName: guild.name, channelName: channel.name, authorID: interaction.user.id, authorName: interaction.user.tag });
};
